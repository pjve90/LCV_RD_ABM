...
\node[state]        (I) [below of=G,node distance=2.1cm,shape=diamond] {RA};
...
\node[state]        (K) [left of=I,shape=diamond,node distance=2.7cm] {RA};
...
(I) edge (G)
(I) edge (J)
(I) edge[bend right=100, min distance=4cm] (M)
(J) edge (F)
(K) edge (I)
;
    \end{tikzpicture}
...
\node[state]        (L) [right of=A,node distance=2.5cm,draw=none,fill=none] {};
\node[state]        (M) [above of=L,node distance=1cm,draw=none,fill=none] {};
\node[state]        (N) [below of=L,node distance=1.65cm,shape=diamond] {RA};
