...
for(d in 1:17){
  for (m in 1:19) {
    for (r in 1:10) {
      ...
      file_path <- sprintf("./Scenario_7/fst_results/results_d%d_m%d_r%d.fst", d, m, r)
      ...
      if (file.exists(file_path)) {
        ...
        if (!dir.exists("./Scenario_7/samples/")) {
          dir.create("./Scenario_7/samples/", recursive = TRUE)
        }
        
        # Save each filtered sample as an .fst file
        output_file <- sprintf("./Scenario_7/samples/raw_sample_d%d_m%d_r%d.fst", d, m, r)
...
input_dir <- "./Scenario_7/samples/"  # Directory containing raw_sample .fst files
output_dir <- "./Scenario_7/lht_lists/"  # Directory for lht_list .fst files
