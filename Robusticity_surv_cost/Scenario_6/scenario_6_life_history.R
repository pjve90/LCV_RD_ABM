# Life cycle variation and resource dynamics ABM: Scenario 6 - life history calculation ----

#Here is the code to calculate the life history tratis from the simulation of Scenario 6, which aims to understand the influence of resource production on the variability of life cycles.
...
for (m in 1:19) {
  for (r in 1:10) {
    ...
    file_path <- sprintf("./Robusticity_surv_cost/Scenario_6/fst_results/results_d%d_m%d_r%d.fst", d, m, r)
    ...
    if (file.exists(file_path)) {
      ...
      if (!dir.exists("./Robusticity_surv_cost/Scenario_6/samples/")) {
        dir.create("./Robusticity_surv_cost/Scenario_6/samples/", recursive = TRUE)
      }
      
      # Save each filtered sample as an .fst file
      output_file <- sprintf("./Robusticity_surv_cost/Scenario_6/samples/raw_sample_d%d_m%d_r%d.fst", d, m, r)
...
input_dir <- "./Robusticity_surv_cost/Scenario_6/samples/"  # Directory containing raw_sample .fst files
output_dir <- "./Robusticity_surv_cost/Scenario_6/lht_lists/"  # Directory for lht_list .fst files
