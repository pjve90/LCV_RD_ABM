...
for (d in 1:17) {
  for (r in 1:10) {
    ...
    file_path <- sprintf("./Scenario_4/fst_results/results_d%d_r%d.fst", d, r)
    ...
    if (file.exists(file_path)) {
      ...
      if (!dir.exists("./Scenario_4/samples/")) {
        dir.create("./Scenario_4/samples/", recursive = TRUE)
      }
      
      # Save each filtered sample as an .fst file
      output_file <- sprintf("./Scenario_4/samples/raw_sample_d%d_r%d.fst", d, r)
...
input_dir <- "./Scenario_4/samples/"  # Directory containing raw_sample .fst files
output_dir <- "./Scenario_4/lht_lists/"  # Directory for lht_list .fst files
