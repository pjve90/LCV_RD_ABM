# Life cycle variation and resource dynamics ABM: Scenario 4 - Code ----

#Here is the code to set up the simulation of Scenario 4, which aims to understand the influence of resource production and habitat quality on the variability of life cycles.

#Production: parameter sweep between 0.1 and 0.9 (17 values)
#Transfers: null
#Habitat quality: double
...
output_dir <- "Robusticity_pop_size/Scenario_4/fst_results"
...
total_tasks <- reps * ncol(prod_prob)  # 10 repetitions for each parameter value (d)
...
for (task_idx in start_task:end_task) {
  r <- ceiling(task_idx / ncol(prod_prob))  # Repetition based on task index
  d <- task_idx %% ncol(prod_prob)  # Parameter value index
...
flattened_data_s4 <- do.call(rbind, results_10_4)
#save flattened data to a .fst file
write.fst(flattened_data_s4,"./Scenario_4/raw_simulation_s4.fst")
#get the row counts for later use
row_counts_s4 <- sapply(results_10_4, nrow)
#save the row counts
saveRDS(row_counts_s4, "./Scenario_4/row_counts_s4.RData")
