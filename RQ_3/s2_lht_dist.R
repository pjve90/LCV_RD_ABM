#Script to plot the distribution of the life history traits in Scenario 2
...
summary_dir <- "./Scenario_2/lht_lists/"
...
for (r in 1:10) {
  ...
  if (file.exists(summary_file)) {
    ...
    temp_df <- get(paste("r", r, sep = "_"))  # current data frame
    temp_df <- rbind(temp_df, new_row_df)  # new row
    assign(paste("r", r, sep = "_"), temp_df)  # updated data frame
  } else {
...
for(r in 1:10) {
  ...
  if (sum(apply(get(paste("r", r, sep = "_")), 2, mean)) > 0) {
    valid_indices <- c(valid_indices, r)  # Store the valid index
  }
}
...
for (r in 1:10) {
  ...
  current_data <- get(paste("r", r, sep = "_"))
...
for(i in 1:length(valid_indices)) {
  r <- valid_indices[i]
  data <- get(paste("r", r, sep = "_"))
  
  # Plot the line for this valid data frame
  lines(0:(ncol(get(paste("r", r, sep = "_")))-1),
        apply(data, 2, mean),
        type = "o",
        col = color_palette[i],
        lty = i,
