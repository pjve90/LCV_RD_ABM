...
### Parameter sweep for one iteration for all the population ----
...
### Parallelisation of parameter sweep ----

#Here, you do the parallelisation of the parameter sweep to check that everything works

#### Parallelisation settings ----

#clean parallel backend to ensure everything is ready for your simulation
...
results <- foreach(d = 1:ncol(prod_prob),
  ...
  return(list(it_data = it_data, it_indpop = it_indpop))
...
#check the population by the end of the iteration
print(head(results[,1])) #it_data
print(head(results[,2]))  #it_indpop
...
logger <- file(get_logger_file_name(), open = "at")

sink(logger, type="message") 

recordlogger <- function(msg, ...)  {
...
for(d in 1:ncol(prod_prob)){
  ...
  recordlogger("Starting simulation for d =", d, "\n")
  ...
  for (b in 1:years){
    ...
    if(nrow(it_indpop)==0){
      ...
      if (b %% 10 == 0) {  # Adjust the modulus for desired frequency
        ...
        recordlogger("d =", d, ": Year", b, "completed\n")
      }
  ...
  recordlogger("Completed simulation for d =", d, "\n\n")
