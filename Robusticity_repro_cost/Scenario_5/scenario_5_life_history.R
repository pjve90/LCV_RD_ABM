...
for (m in 1:19) {
  for (r in 1:10) {
    ...
    file_path <- sprintf("./Robusticity_repro_cost/Scenario_5/fst_results/results_m%d_r%d.fst", m, r)
    ...
    if (file.exists(file_path)) {
      ...
      if (!dir.exists("./Robusticity_repro_cost/Scenario_5/samples/")) {
        dir.create("./Robusticity_repro_cost/Scenario_5/samples/", recursive = TRUE)
      }
      
      # Save each filtered sample as an .fst file
      output_file <- sprintf("./Robusticity_repro_cost/Scenario_5/samples/raw_sample_m%d_r%d.fst", m, r)
...
batch_process <- function(batch_index, input_dir, output_dir, chunk_size) {
  ...
  all_files <- list.files(input_dir, pattern = "raw_sample_m[0-9]+_r[0-9]+\\.fst$", full.names = TRUE)
  ...
  for (file_path in all_files[start_idx:end_idx]) {
    ...
    matches <- regexpr("m([0-9]+)_r([0-9]+)", basename(file_path))
    if (matches[1] != -1) {
      ...
      m_value <- as.integer(sub("m([0-9]+)_r([0-9]+)", "\\1", match_values))
      r_value <- as.integer(sub("m([0-9]+)_r([0-9]+)", "\\2", match_values))
      ...
      if (nrow(raw_sample) == 0) {
        ...
        output_file <- file.path(output_dir, sprintf("lht_list_m%d_r%d.fst", m_value, r_value))
...
input_dir <- "./Robusticity_repro_cost/Scenario_5/samples/"  # Directory containing raw_sample .fst files
output_dir <- "./Robusticity_repro_cost/Scenario_5/lht_lists/"  # Directory for lht_list .fst files
...
total_files <- length(list.files(input_dir, pattern = "raw_sample_m[0-9]+_r[0-9]+\\.fst$"))
