...
for (d in 1:17) {
  for (r in 1:10) {
    ...
    if (file.exists(file_path)) {
      ...
      if (!dir.exists("./Scenario_1/samples/")) {
        dir.create("./Scenario_1/samples/", recursive = TRUE)
      }
      
      # Save each filtered sample as an .fst file
      output_file <- sprintf("./Scenario_1/samples/raw_sample_d%d_r%d.fst", d, r)
...
input_dir <- "./Scenario_1/samples/"  # Directory containing raw_sample .fst files
output_dir <- "./Scenario_1/lht_lists/"  # Directory for lht_list .fst files
