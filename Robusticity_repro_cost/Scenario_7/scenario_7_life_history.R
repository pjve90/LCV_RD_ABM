# Life cycle variation and resource dynamics ABM: Scenario 7 - life history calculation ----

#Here is the code to calculate the life history tratis from the simulation of Scenario 7, which aims to understand the influence of resource production on the variability of life cycles.
...
for(d in 1:17){
  for (m in 1:19) {
    for (r in 1:10) {
      ...
      file_path <- sprintf("./Robusticity_repro_cost/Scenario_7/fst_results/results_d%d_m%d_r%d.fst", d, m, r)
      ...
      if (file.exists(file_path)) {
        ...
        if (!dir.exists("./Robusticity_repro_cost/Scenario_7/samples/")) {
          dir.create("./Robusticity_repro_cost/Scenario_7/samples/", recursive = TRUE)
        }
        
        # Save each filtered sample as an .fst file
        output_file <- sprintf("./Robusticity_repro_cost/Scenario_7/samples/raw_sample_d%d_m%d_r%d.fst", d, m, r)
...
batch_process <- function(batch_index, input_dir, output_dir, chunk_size) {
  ...
  all_files <- list.files(input_dir, pattern = "raw_sample_d[0-9]+_m[0-9]+_r[0-9]+\\.fst$", full.names = TRUE)
  ...
  for (file_path in all_files[start_idx:end_idx]) {
    ...
    matches <- regexpr("d([0-9]+)_m([0-9]+)_r([0-9]+)", basename(file_path))
    if (matches[1] != -1) {
      # The result of regmatches should be accessed with the `matches` object
      match_values <- regmatches(basename(file_path), matches)
      # Split the match_values to get m_value and r_value
      d_value <- as.integer(sub("d([0-9]+)_m([0-9]+)_r([0-9]+)", "\\1", match_values))
      m_value <- as.integer(sub("d([0-9]+)_m([0-9]+)_r([0-9]+)", "\\1", match_values))
      r_value <- as.integer(sub("d([0-9]+)_m([0-9]+)_r([0-9]+)", "\\2", match_values))
      ...
      if (nrow(raw_sample) == 0) {
        ...
        output_file <- file.path(output_dir, sprintf("lht_list_d%d_m%d_r%d.fst", d_value, m_value, r_value))
...
input_dir <- "./Robusticity_repro_cost/Scenario_7/samples/"  # Directory containing raw_sample .fst files
output_dir <- "./Robusticity_repro_cost/Scenario_7/lht_lists/"  # Directory for lht_list .fst files
...
total_files <- length(list.files(input_dir, pattern = "raw_sample_d[0-9]+_m[0-9]+_r[0-9]+\\.fst$"))
