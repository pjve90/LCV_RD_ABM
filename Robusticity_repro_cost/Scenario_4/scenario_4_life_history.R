# Life cycle variation and resource dynamics ABM: Scenario 4 - life history calculation ----

#Here is the code to calculate the life history tratis from the simulation of Scenario 4, which aims to understand the influence of resource production on the variability of life cycles.
...
for (d in 1:17) {
  for (r in 1:10) {
    ...
    file_path <- sprintf("./Robusticity_repro_cost/Scenario_4/fst_results/results_d%d_r%d.fst", d, r)
    ...
    if (file.exists(file_path)) {
      ...
      if (!dir.exists("./Robusticity_repro_cost/Scenario_4/samples/")) {
        dir.create("./Robusticity_repro_cost/Scenario_4/samples/", recursive = TRUE)
      }
      
      # Save each filtered sample as an .fst file
      output_file <- sprintf("./Robusticity_repro_cost/Scenario_4/samples/raw_sample_d%d_r%d.fst", d, r)
...
batch_process <- function(batch_index, input_dir, output_dir, chunk_size) {
  ...
  all_files <- list.files(input_dir, pattern = "raw_sample_d[0-9]+_r[0-9]+\\.fst$", full.names = TRUE)
  ...
  for (file_path in all_files[start_idx:end_idx]) {
    ...
    matches <- regexpr("d([0-9]+)_r([0-9]+)", basename(file_path))
    if (matches[1] != -1) {
      ...
      d_value <- as.integer(sub("d([0-9]+)_r([0-9]+)", "\\1", match_values))
      r_value <- as.integer(sub("d([0-9]+)_r([0-9]+)", "\\2", match_values))
      ...
      if (nrow(raw_sample) == 0) {
        ...
        output_file <- file.path(output_dir, sprintf("lht_list_d%d_r%d.fst", d_value, r_value))
...
input_dir <- "./Robusticity_repro_cost/Scenario_4/samples/"  # Directory containing raw_sample .fst files
output_dir <- "./Robusticity_repro_cost/Scenario_4/lht_lists/"  # Directory for lht_list .fst files
...
total_files <- length(list.files(input_dir, pattern = "raw_sample_d[0-9]+_r[0-9]+\\.fst$"))
