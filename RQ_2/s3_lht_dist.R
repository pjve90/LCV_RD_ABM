...
summary_dir <- "./SCenario_3/lht_lists/"
...
for (m in 1:19) {
  ...
  for (r in 1:10) {
    ...
    summary_file <- file.path(summary_dir, sprintf("lht_list_m%d_r%d.fst", m, r))
    
    # Check if the summary file exists
    if (file.exists(summary_file)) {
      # Read the summary file
      results_s3 <- read_fst(summary_file)
      
      # Extract the longevity (assuming it's stored under a column "lng")
      lng_vector <- results_s3$lng
...
for (m in 1:19) {
  ...
  current_data <- get(paste("m", m, sep = "_"))
...
legend(x = 105, y = (max_y + 1),
       title = "Transfers\nprobabilities",
       legend = transf_prob[ valid_indices], # Use valid indices for the legend
