...
mat_invest <- function(it_indpop){
  if(it_mompop$mom_surplus_a[i] > 0){ #if the current individual is a mother of any descendants currently alive that need resources, and if she has a surplus of resources
    ...
    it_indpop$res_a[it_mompop$id[i] %in% it_indpop$id] <- it_indpop$res_a[it_mompop$id[i] %in% it_indpop$id] - it_mompop$mom_surplus_a[i]
    it_indpop$res_a[it_indpop$id[i] %in% it_descpop$id] <- it_indpop$res_a[it_indpop$id %in% it_descpop$id[i]] - it_descpop$res_a[i]
  }
