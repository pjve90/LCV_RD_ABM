...
desc_need <- function(it_indpop){
  if(it_indpop$mom_id[i] %in% it_indpop$id == T & it_indpop$stage[i]==1 & it_indpop$res_a[i] < surv_cost){ #if your mother is alive, you are a juvenile, and don't have enough resources to cover the costs of survival
    it_indpop$desc_need[i] <- 1
  } else 
    if(it_indpop$mom_id[i] %in% it_indpop$id == T & it_indpop$stage[i]==1 & it_indpop$res_a[i] >= surv_cost){
