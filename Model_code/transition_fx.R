...
transition <- function(it_indpop){
  if(it_indpop$stage[i]==1 & it_indpop$age[i] >= 10 & it_indpop$store_a[i] >= surv_cost+repro_cost){ #transition juvenile to adult
    ...
    if(it_indpop$stage[i]==2 & it_indpop$store_a[i] >= surv_cost+repro_cost){ #transition adult to reproductive career
        it_indpop$store_a[i] <- it_indpop$store_a[i] - repro_cost
        it_indpop$repro[i] <- 1
        it_indpop$stage[i] <- 3
        it_indpop$tlr[i] <- 0
      ...
      if(it_indpop$stage[i]==2 & it_indpop$age[i]>=60 ){ #forced transition adult to post-reproductive
        ...
        if(it_indpop$stage[i]==3 & it_indpop$age[i] >=40 & it_indpop$store_a[i] <= repro_cost & it_indpop$tlr[i] >= 10){ #transition reproductive career to post-reproductive career
          it_indpop$stage[i] <- 4
        }
         else
