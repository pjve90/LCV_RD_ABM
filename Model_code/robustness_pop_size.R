# Life cycle variation and resource dynamics ABM: Baseline model - robustness check: population size ----

#Here is the code to set up the simulation for the robustness check of population size with the baseline model.
...
source("./Model_code/transfers_blockprobs_fx_s2.R")
...
results_10_b <- foreach(r=1:10,
    ...
    for (b in 1:years){
      ...
      if(nrow(it_indpop)==0){
        ...
        if (b %% 10 == 0) {
          sink(log_file, append = TRUE)
          cat(paste("Year", b, "completed in repetition =", r, "at", Sys.time(), "\n"))
          sink()
        }
    ...
    sink(log_file, append = TRUE)
    cat(paste("Completed simulation in repetition =", r, "at", Sys.time(), "\n"))
    sink()
    ...
    sink(log_file, append = TRUE)
    cat(paste("Length of simulation in repetition =", r, "is", time_sim, "minutes", "\n"))
    sink()
    
    # Return the result with a clear name indicating the combination of d and r
    setNames(list(it_dataf), paste0("r_", r))
