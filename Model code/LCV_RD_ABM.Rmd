...
survive <- function(it_indpop){
  if(it_indpop$prod_a[i] >= surv_c & runif(1,min=0,max=1)<(surv_p+log(it_indpop$prod_a[i]+1)/20)){
  it_indpop$surv[i] <- 1
  }
  else{
  it_indpop$surv[i] <- 0
...
reproduce <- function(it_indpop){
  if(it_indpop$prod_a[i] >= repro_c & runif(1,min=0,max=1)<(repro_p+log(it_indpop$prod_a[i]+1)/20)) {
  it_indpop$repro[i] <- 1
  }else{
  it_indpop$repro[i] <- 0
  }
