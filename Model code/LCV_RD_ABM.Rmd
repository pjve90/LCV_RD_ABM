...
The functions for state variables are defined to determine the dynamics each individual can experience in one year (i.e. one iteration). The functions for survival and age are defined first, followed by the discount on the amount of resources available due to the costs of survival. An individual can either survive (1) or not (0) in the survival function. An individual can survive (1) depending if the individual has enough resources to cover the age-specific costs of survival (`surv_c_age`) and samples from a uniform distribution a value lower than the sum of the probability of surviving (`surv_p`) and the logarithm of the amount of resources available (`prod_a`). The influence of the amount of resources available is escaled to avoid an explosive population growth.
...
The functions for the reproductive dynamics are defined here. First the probability of reproducing is defined, followed by the reproductive costs. The reproduction function tells if an individual reproduces (1) or not (0). An individual reproduces (1) depending if the individual has enough resources to cover the age-specific costs of reproduction (`repro_c_age`)and samples from a uniform distribution a value lower than the sum of the probability of reproducing (`repro_p`) and the logarithm of the amount of resources available (`prod_a`). The influence of the amount of resources available is escaled to avoid an explosive population growth.
...
- Average interbirth interval (IBI): Number of reproductive years per descendant.
$$IBI=\frac{ALR-AFR}{LRO}$$

```{r}
#longevity
longevity <- function(final_ind_data){
  final_ind_data$age[i] <- as.numeric(rowSums(it_data[i,grep("surv",colnames(it_data))],na.rm = T))
  return(final_ind_data$age)
}
...
#IBI
ibi <- function(final_ind_data){
  final_ind_data$ibi[i] <- (final_ind_data$alr[i]-final_ind_data$afr[i])/final_ind_data$lro[i]
  return(final_ind_data$ibi)
}
```
...
consumption_ev <- function(final_ind_data){
  final_ind_data$cons_ev[i] <- final_ind_data$age[i]
  return(final_ind_data$cons_ev)
}

#resource consumption
consumption_res <- function(final_ind_data){
  final_ind_data$cons_res[i] <- final_ind_data$cons_env[i]*cons
  return(final_ind_data$cons_res)
}

#resources available
resources_av <- function(final_ind_data){
  final_ind_data$res_av[i] <- as.numeric(rowSums(it_data[i,grep("prod_a",colnames(it_data))],na.rm=T))
  return(final_ind_data$res_av)
  }
```
...
for(i in 1:nrow(it_data)){
  #life-history traits
  #longevity
final_ind_data$age <- longevity(final_ind_data)
  #lro
...
final_ind_data$alr <- alr(final_ind_data)
  #ibi
final_ind_data$ibi <- ibi(final_ind_data)
  #resource dynamics
  #production events
...
final_ind_data$cons_res <- consumption_res(final_ind_data)
  #resources available
final_ind_data$res_av <- resources_av(final_ind_data)
}

#plot it
#life-history traits and resources available
layout(matrix(c(1,1,2,2,3,3,0,4,4,5,5,0),nrow=2,ncol=6,byrow=T))
plot(final_ind_data$age~final_ind_data$res_av)
plot(final_ind_data$lro~final_ind_data$res_av)
plot(final_ind_data$afr~final_ind_data$res_av)
plot(final_ind_data$alr~final_ind_data$res_av)
plot(final_ind_data$ibi~final_ind_data$res_av)
#life-history traits and resource production
layout(matrix(c(1,1,2,2,3,3,0,4,4,5,5,0),nrow=2,ncol=6,byrow=T))
plot(final_ind_data$age~final_ind_data$prod_res)
plot(final_ind_data$lro~final_ind_data$prod_res)
plot(final_ind_data$afr~final_ind_data$prod_res)
plot(final_ind_data$alr~final_ind_data$prod_res)
plot(final_ind_data$ibi~final_ind_data$prod_res)
