...
survive <- function(it_indpop){
  if(it_indpop$prod_a[i] >= surv_c_age[it_indpop$age[i]+1] & runif(1,min=0,max=1)>(die_p+log(it_indpop$prod_a[i]+1)/20)){
  it_indpop$surv[i] <- 1
  }
  else{
  it_indpop$surv[i] <- 0
...
-   Life cycle transition probability (`trans_p`) - juvenile only.
-   Age-specific life cycle transition probability (`trans_p_age`) - juvenile only.
-   Life cycle transition cost (`trans_c`) - juvenile only.
...
die_p_age_juv <- surv_p_juv*exp(log(surv_p_juv)/2*c(1:101))
#cost of surviving
surv_c_juv <- 0.25
#age-specific cost of surviving
surv_c_age_juv <- c(surv_c_juv,surv_c_juv*exp(exp(surv_c_juv)/20*c(1:100))) 

#transition
#probability
trans_p_juv <- 0.5
#age-specific probability of transition
trans_p_age_juv <- c(trans_p_juv,trans_p_juv*exp(exp(trans_p_juv)/40*c(1:100)))
#cost of transition
trans_c_juv <- 4
...
#probability of dying
die_p_rc <- 0.25
#age-specific proability of dying
die_p_age_rc <- c(die_p_rc,die_p_rc*exp(exp(die_p_rc)/93*c(1:100)))
#cost of surviving
surv_c_rc <- 0.25
#age-specific cost of surviving
surv_c_age_rc <- c(surv_c_rc,surv_c_rc*exp(exp(surv_c_rc)/20*c(1:100))) 
...
repro_c_rc <- 0.25
#age-specific cost of reproduction
repro_c_age_rc <- c(repro_c_rc,repro_c_rc*exp(exp(repro_c_rc)/20*c(1:100)))
#number of descendants per reproduction
repro_n_rc <- 1
...
#reproductive career
survive_rc <- function(it_indpop){
  if(it_indpop$store_a[i] >= surv_c_age_rc[it_indpop$age[i]+1] & runif(1,min=0,max=1)>(die_p_age_rc+log(it_indpop$store_a[i]+1)/20)){
  it_indpop$surv[i] <- 1
  }
  else{
  it_indpop$surv[i] <- 0
...
#discount survival costs
#juvenile
survive_c_j <- function(it_indpop) {
    it_indpop$prod_a[i] <- it_indpop$prod_a[i]-surv_c_age_juv[it_indpop$age[i]+1]
    return(it_indpop$store_a)
}
#reproductive career
survive_c_rc <- function(it_indpop) {
    it_indpop$store_a[i] <- it_indpop$store_a[i]-surv_c_age_rc[it_indpop$age[i]+1]
    return(it_indpop$store_a)
}
```
...
#production
#juvenile
produce_j <- function(it_indpop){
  it_indpop$prod_o[i] <- rbinom(1,1,prod_p_juv)
  return(it_indpop$prod_o)
}
#reproductive career
produce_rc <- function(it_indpop){
